...
function findPostURL(formData) {
  ...
  formData.forEach((data, _) => {
    ...
    const matched = data.match(/https:\/\/bsky.(app|social)\/profile\/[a-zA-Z0-9\.-]+\/post\/\w+/) ?? [];
    url = url ?? matched[0];
  });
  return url;
}
...
/** @type {ServiceWorkerGlobalScope} */ (self).addEventListener('fetch', (event) => {
        ...
        if (postURL) {
          ...
          switch (json.error) {
            case 'already bookmarked':
              return Response.redirect(`/confirm.html?url=${encodeURIComponent(postURL)}`);
            default:
              return Response.redirect(`/error.html?${encodeURIComponent(json.error)}`);
          }
        }
